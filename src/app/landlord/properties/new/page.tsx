    ...
    <div className="min-h-screen bg-[#E5E0D8] py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="px-3 py-1 rounded bg-[#B86A36]/20 text-[#8A4A20] text-xs font-mono font-bold border border-[#B86A36]/30">
            Free Landlord Listing
          </span>
        </div>

        {/* Wizard Stepper Bar */}
        <div className="bg-[#FAFAFA] p-4 rounded-xl border border-[#E5E0D8] shadow-sm">
            ...
            {[
              ...
              <button
                ...
                className={`py-2 px-1 rounded-lg border transition-all ${
                  step === s.num
                    ? 'bg-[#231F20] text-white border-[#231F20] font-bold shadow'
                    : step > s.num
                    ? 'bg-[#E5E0D8] text-[#231F20] border-[#231F20]/20'
                    : 'bg-white/50 text-[#908682] border-transparent'
                }`}
              >
                {s.label}
              </button>
        ...
        {step === 1 && (
          <div className="bg-[#FAFAFA] rounded-xl border border-[#E5E0D8] p-6 sm:p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-4 font-mono text-xs">
              <div>
                <label className="block text-[#3D3531] mb-1 font-semibold">Property Title</label>
                <input
                  type="text"
                  value={title}
            ...
            <div className="pt-4 flex justify-end">
              <button
                type="button"
                onClick={() => setStep(2)}
                className="px-6 py-3 rounded-md bg-[#231F20] text-white font-mono text-xs font-bold hover:bg-[#3D3531] transition-colors flex items-center gap-2"
              >
        ...
        {step === 2 && (
          <div className="bg-[#FAFAFA] rounded-xl border border-[#E5E0D8] p-6 sm:p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-3 font-mono text-xs">
              <label className="block font-semibold text-[#231F20]">Select Cover Photograph Preset</label>
                ...
                {SAMPLE_IMAGE_PRESETS.map((img, idx) => (
                  <button
                    ...
                    className={`relative h-24 rounded-lg overflow-hidden border-2 transition-all ${
                      selectedImage === img
                        ? 'border-[#B86A36] scale-105 shadow-md'
            ...
            {/* Amenities list & add */}
            <div className="space-y-3 font-mono text-xs">
              <label className="block font-semibold text-[#231F20]">Property Amenities</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 p-2.5 rounded-lg bg-white border border-[#E5E0D8] text-[#231F20]"
                />
                <button
                  type="button"
                  onClick={handleAddAmenity}
                  className="px-4 py-2 rounded-lg bg-[#B86A36] text-white font-bold"
                >
                  + Add
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {amenities.map((amenity, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-[#E5E0D8] text-[#231F20]"
                  >
          ...
          <div className="bg-[#FAFAFA] rounded-xl border border-[#E5E0D8] p-6 sm:p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-5 font-mono text-xs">
              ...
              <div className="p-4 rounded-xl bg-white border border-[#E5E0D8] space-y-2">
                <div className="flex items-center justify-between">
                  <label className="font-bold text-[#231F20] text-sm">
                    Minimum Annual Income Requirement ($)
                  </label>
                  <span className="text-[#4A6B32] font-bold text-base">
                ...
                <p className="text-[11px] text-[#3D3531]">
                  Tenants will prove <code className="text-[#231F20]">Income ≥ ${minIncome.toLocaleString()}</code> without disclosing actual salary.
                </p>
              </div>

              {/* Background Requirement */}
              <div className="p-4 rounded-xl bg-white border border-[#E5E0D8] flex items-center justify-between">
                <div>
              ...
              <div className="p-4 rounded-xl bg-[#E5E0D8] border border-[#231F20]/10 flex items-center justify-between">
                ...
                <span className="font-bold text-[#231F20] text-base">${verificationFee.toFixed(2)}</span>
              </div>
            </div>
        ...
        {step === 4 && (
          <div className="bg-[#FAFAFA] rounded-xl border border-[#E5E0D8] p-6 sm:p-8 space-y-6 shadow-sm">
            <div>
              <span className="px-3 py-1 rounded bg-[#B86A36]/20 text-[#8A4A20] text-xs font-mono font-bold border border-[#B86A36]/30 inline-block mb-2">
            ...
            {/* Summary Card */}
            <div className="p-5 rounded-xl bg-[#231F20] text-white space-y-4 font-mono text-xs">
              <div className="flex justify-between items-start pb-3 border-b border-white/10">
                <div>
                  <h3 className="font-serif text-xl font-bold text-[#00A8E8]">{title}</h3>
                  <p className="text-[#908682]">{address}, {city}, {state} {zip}</p>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-serif font-bold text-[#B86A36]">
                    ${price.toLocaleString()}
                  </div>
                  <span className="text-[10px] text-[#908682]">/ month</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5 pt-1">
                <span className="text-[#00A8E8] font-bold">Midnight ZK Qualification Rules:</span>
            ...
            <div className="pt-4 flex justify-between">
              ...
              <button
                ...
                className="px-8 py-3.5 rounded-md bg-[#B86A36] hover:bg-[#A05A2C] text-white font-mono text-xs font-bold shadow-lg transition-all flex items-center gap-2"
              >
